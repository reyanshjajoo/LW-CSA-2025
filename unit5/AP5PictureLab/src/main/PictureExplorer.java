...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener {
    ...
    /** lock for synchronizing explorer window closing */
    ...
    private void createAndInitPictureFrame() {
        ...
        pictureFrame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE); // closing is going to be handled manually
        pictureFrame.addWindowListener(new java.awt.event.WindowAdapter() {
            @Override
            public void windowClosing(java.awt.event.WindowEvent windowEvent) {
                pictureFrame.dispose();
                // signal the lock if it exists
                if (lock != null) {
                    lock.countDown();
                }
            }
     ...
     */
    private JPanel createLocationPanel(Font labelFont) {
    ...
    /**
     * Blocks the caller until the window is disposed
     */
    public void await() {
        try {
            lock.await();
